...
definition "WHILEIT I b c s = whileT b c s"
      ...
      focus (rule hnr_If)
        ...
        focus (rule hnr_bind)
          ...
          focus (rule hn_refine_frame)
            applyS rprems
            apply  (rule enttI; solve_entails)
          solved
...
schematic_goal
  ...
  unfolding assms
  apply(subst WHILEIT_def[symmetric, where I=I])
  apply(subst WHILEIT_to_monadic)
