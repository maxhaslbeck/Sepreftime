...
\<longleftrightarrow> (\<forall>b bi a ai. P (a,b) \<longrightarrow>
  ...
  lemma hr_comp_Id1[simp]: "hr_comp (pure Id) R = pure R"  
    ...
    apply (intro ext ent_iffI)    
     apply (smt BNF_Greatest_Fixpoint.IdD SLTC_More.mod_pure_star_dist entails_def entails_ex move_back_pure)
  ...
  lemma hr_comp_Id2[simp]: "hr_comp R Id = R"  
    ...
    apply (intro ext ent_iffI)
    apply (smt BNF_Greatest_Fixpoint.IdD SLTC_More.mod_pure_star_dist entails_def entails_ex)
...
lemma entails_pure''': "(emp \<Longrightarrow>\<^sub>A \<up> B) = B"  
  ...
  lemma hr_comp_assoc: "hr_comp (hr_comp R S) T = hr_comp R (S O T)"
    ...
    subgoal by (smt SLTC_More.mod_pure_star_dist entailsI mod_ex_dist relcomp.relcompI) 
    subgoal 
      apply (clarsimp intro!: ent_ex_preI entails_pure'' simp:  pure_conj[symmetric] del: pure_conj)
      apply (rule ent_ex_postI)
      apply (rule ent_ex_postI)
