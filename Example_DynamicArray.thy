...
definition "mop_empty_list = SPECT [ [] \<mapsto> enat 12 ]"

term list_assn


lemma new[sepref_fr_rules]: "hn_refine emp dyn_array_new emp dyn_array (mop_empty_list)"
...
sepref_definition test is "uncurry0 (do { xs \<leftarrow>mop_empty_list;  mop_push_list 10 (0::nat) xs })" :: "unit_assn\<^sup>k \<rightarrow>\<^sub>a dyn_array"
  apply sepref_dbg_keep                   
  apply sepref_dbg_trans_keep                  
        apply sepref_dbg_trans_step_keep oops

sepref_definition test is "uncurry0 (do { xs \<leftarrow>mop_empty_list;  mop_push_list 23 (0::nat) xs })" :: "unit_assn\<^sup>k \<rightarrow>\<^sub>a dyn_array"
